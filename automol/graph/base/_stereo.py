...
def _local_atom_stereo_corrected_geometry(gra, atm_par_dct, geo, geo_idx_dct=None):
    ...
    for atm_key in ste_atm_keys:
        ...
        if curr_par != par:
            ...
            if len(atm_piv_keys) == 2:
                ...
                # do the rotation
                rot_axis = util.vec.unit_bisector(atm3_xyz, atm4_xyz, orig_xyz=atm_xyz)

                rot_atm_keys = branch_atom_keys(
                    gra, atm_key, atm3_key
                ...
                # do the rotation
                rot_axis = util.vec.unit_bisector(atm1_xyz, atm2_xyz, orig_xyz=atm_xyz)
...
def _local_bond_stereo_corrected_geometry(gra, bnd_par_dct, geo, geo_idx_dct=None):
    ...
    for bnd_key in bnd_keys:
        ...
        if curr_par != par:
            ...
            geo = geometry_rotate_bond(
                gra, geo, [atm1_key, atm2_key], numpy.pi, geo_idx_dct=geo_idx_dct
            )
...
def _connected_expand_stereo_with_priorities_and_amchis(gra):
    ...
    for gra_, pri_dct, _ in diast_gpls:
        chi, _ = connected_amchi_with_indices(gra_, pri_dct=pri_dct, is_refl=None)
        gpcs.append((gra_, pri_dct, chi))

    for (ugra, upri_dct), (rgra, rpri_dct) in enant_gps:
        uchi, _ = connected_amchi_with_indices(ugra, pri_dct=upri_dct, is_refl=False)
        rchi, _ = connected_amchi_with_indices(ugra, pri_dct=upri_dct, is_refl=True)
