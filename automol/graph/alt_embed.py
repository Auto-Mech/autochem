...
def heuristic_torsion_angle_distance_range(gra, key1, key2, key3, key4,
    ...
    :param cis: whether to restrict the angle to cis; if `False` the angle will
        be restricted to trans; if `None` it will not be restricted
...
def path_distance_bounds_(gra):
    ...
    def _distance_bounds(path):
        ...
        if len(path) == 1:
            ...
            if rsz is None:
                ldist, udist = heuristic_torsion_angle_distance_range(
                ...
                oldist, oudist = heuristic_torsion_angle_distance_range(
                    gra, *path)
                ldist = min(rldist, oldist)
                udist = max(rudist, oudist)
