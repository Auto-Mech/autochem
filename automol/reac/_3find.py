...
def arbitrary_reactions(
    ...
    def _set_bimol_direction(rct_gras, prd_gras): 
        ...
        if bimol_dir < 0:
            prd_gra =  pgra = graph.union_from_sequence(rct_gras)
            rct_gra = graph.union_from_sequence(prd_gras)
        else:
            rct_gra =  pgra = graph.union_from_sequence(rct_gras)
            prd_gra = graph.union_from_sequence(prd_gras)
        ...
        for common_iso_dct in common_fragments: 
            ...
            for key in common_atm_keys:
                ...
                if all(adj_atm in common_iso_dct.keys() for adj_atm in adj_atms):
                    ...
                    if set(prd_adj_atms) == set(graph.atom_neighbor_atom_keys(prd_gra, common_iso_dct[key])):
                        true_common_atm_keys.append(key)

            # set the hydrogens to explicit so we can break/form them, but not if they are bound
            # to a common atm
            common_rct_gra = graph.explicit(rct_gra, atm_keys=set(graph.atom_keys(rct_gra))-set(true_common_atm_keys))
            common_prd_gra = graph.explicit(
                prd_gra, 
                atm_keys=set(graph.atom_keys(prd_gra))-set([common_iso_dct[atm] for atm in true_common_atm_keys]))
            ...
            true_common_bnd_keys = frozenset(
                [key for key in shared_bnd_keys if len(graph.unsaturated_atom_keys(common_rct_gra) & key) == 0])

            cur_overlap_perc = len(common_bnd_keys)/len(graph.bond_keys(graph.implicit(common_rct_gra)))
    ...
    def _edge_connects_different_groups(frm_bnd):
        ...
        for i in range(len(atom_sets)):
            for j in range(i + 1, len(atom_sets)):
                if (frm_bnd & atom_sets[i]) and (frm_bnd & atom_sets[j]):
                    return True
                if (frm_bnd & atom_sets[j]) and (frm_bnd & atom_sets[i]):
                    return True
        return False
        ...
        for (num_brk, num_frm) in brk_frm_combos:
            ...
            for pot_frm_bnd_combo in itertools.combinations(frm_bnd_keys, num_frm):
                ...
                for brk_bnd_combo in itertools.combinations(brk_bnd_keys, num_brk):
                    ...
                    inv_dct = graph.isomorphism(pot_rct_gra, stable_prd_gra, stereo=False)
                    if inv_dct:
                        if bimol_dir < 0:
                            # reverse back
                            tmp_frm_bnd_combo =  tuple(
                                frozenset(inv_dct[atm] for atm in bnd) for bnd in brk_bnd_combo)
                            brk_bnd_combo =  tuple(
                                frozenset(inv_dct[atm] for atm in bnd) for bnd in frm_bnd_combo)
                            frm_bnd_combo = tmp_frm_bnd_combo
                            inv_dct = {value: key for key, value in inv_dct.items()}
                        print('found! breaking', brk_bnd_combo, 'forming', frm_bnd_combo)
                        break   
