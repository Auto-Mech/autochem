...
def transmission_coefficient(ene, alpha, rxn_freq):
    """
    """

    kes = ()
    for ene in enes:
        kes += (_transmission_coefficient(ene, alpha, rxn_freq),)

    return kes


def action(enes, alpha, rxn_freq):
    """ Calculate action at seve
    """

    ses = ()
    for ene in enes:
        ses += (_action(ene, alpha, rxn_freq),)

    return ses


def _transmission_coefficient(ene, alpha, rxn_freq):
    ...
    if ene < 0.0:
        denom = math.exp(action(-1.0*ene, alpha, rxn_freq))
    else:
...
def _action(ene, alpha, rxn_freq):
...
def alpha(freqs, cfc_dct, qfc_dct=None):
    ...
    if vr4 is not None:
        _alpha -= (numpy.pi / 8.0) * (
            cfc_dct[(ridx, ridx, ridx, ridx)] / rxn_freq**6)
