...
def test__highd_mat():
    ...
    with open(os.path.join(DAT_PATH, 'ch4_h.cubic_submat'), mode='r', errors='ignore', encoding='utf-8') as fobj:
        ref_3d_submat_str = fobj.read()

    ref_4d_submat_str = None
    with open(os.path.join(DAT_PATH, 'ch4_h.quartic_submat'), mode='r', errors='ignore', encoding='utf-8') as fobj:
