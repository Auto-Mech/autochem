...
def test__vec():
    """ test automol.cart.vec
    """

    # Test angle calculators
    ref_perp = (-0.90180687, -0.40614043, -0.14762896)
    perp = cart.vec.unit_perpendicular(MAT[0], MAT[1], allow_parallel=False)
    assert numpy.allclose(perp, ref_perp)
    ref_perp = (0.000, 0.000, 0.000)
    perp = cart.vec.unit_perpendicular(MAT[0], MAT[1], allow_parallel=True)
    assert numpy.allclose(perp, ref_perp)
    with pytest.raises(ValueError):
        cart.vec.unit_perpendicular(MAT[0], MAT[0], allow_parallel=False)

    ref_angle = 0.28211376550390677
    angle = cart.vec.projected_central_angle(MAT[0], MAT[1], MAT[2])
    assert numpy.isclose(angle, ref_angle)

    # Test the string writer
    ref_vec_str = read_file(['data'], 'vec.dat')
    vec_str = cart.vec.string((MAT[0] + MAT[1]), num_per_row=3)

    assert vec_str == ref_vec_str


def test__mat():
    """ test automol.cart.mat
    """
    ...
    dist_mat = cart.mat.distance_matrix(MAT)
    assert numpy.allclose(dist_mat, ref_dist_mat)

    rand_rot_mat = cart.mat.random_rotation_matrix()
    ...
    rot_mat = cart.mat.rotation_matrix((1.0, 0.0, 0.0), 30.0/numpy.pi)
    ...
    axis_align_mat = cart.mat.axis_alignment_matrix(MAT[0], MAT[1])
    ...
    xyz1 = (MAT[0], MAT[1])
    xyz2 = (MAT[2], MAT[3])
    superimp_mat = cart.mat.superimposition_matrix(
