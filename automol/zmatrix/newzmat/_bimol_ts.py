...
from automol.zmatrix.newzmat._util import join_atom_keys
from automol.zmatrix.newzmat._util import include_babs3
from automol.zmatrix.newzmat._util import reorder_zma_for_radicals
from automol.zmatrix.newzmat._util import shift_vals_from_dummy
from automol.zmatrix.newzmat._util import rct1_x_join
from automol.zmatrix.newzmat._util import rct1x_rct2_join
from automol.zmatrix.newzmat import _const as constbuild
...
def insertion(rct_zmas, prd_zmas, tras):
    ...
    rct1_x_zma = rct1_x_join(
        rct1_zma, rct1_atm1_key, rct1_atm2_key, rct1_atm3_key)
    ts_zma = rct1x_rct2_join(
        rct1_x_zma, rct2_zma,
        dist_name, dist_val,
...
def hydrogen_abstraction(rct_zmas, prd_zmas, tras, sigma=False):
    """ z-matrix for an abstraction reaction
    """
    if sigma:
        ret = _sigma_hydrogen_abstraction(rct_zmas, prd_zmas, tras)
    else:
        ret = _hydrogen_abstraction(rct_zmas, prd_zmas, tras)
    return ret
