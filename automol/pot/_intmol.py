...
from automol.geom import count, distance, symbols
...
def low_repulsion_struct(ref_geo, test_geo, potential="exp6", thresh=40.0):
    ...
    ref_pot = intramol_interaction_potential_sum(ref_geo, potential=potential)
    test_pot = intramol_interaction_potential_sum(test_geo, potential=potential)
...
def pairwise_potential_matrix(geo, potential="exp6"):
    ...
    for i in range(natoms):
        for j in range(natoms):
            pot_mat[i, j] = _pairwise_potentials(geo, (i, j), potential=potential)
...
def _pairwise_potentials(geo, idx_pair, potential="exp6"):
    ...
    assert potential in ("exp6", "lj_12_6"), f"potential {potential} != exp6 or lj_12_6"
